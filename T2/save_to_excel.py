...
#                 worksheet.cell(row=row, column=max_col).border = Border(right=bold_side, top=bold_side, bottom=bold_side)
#
...
def save_to_excel(results_df, output_excel_file_path, rois):
    # Move the 'Overall Result' column to be right after the 'Patient ID' column
    columns = list(results_df.columns)
    overall_result_index = columns.index('Overall Result')

    # Remove 'Overall Result' from its original position
    columns.pop(overall_result_index)

    # Insert 'Overall Result' after 'Patient ID'
    patient_id_index = columns.index('Patient ID')
    columns.insert(patient_id_index + 1, 'Overall Result')
    ...
    with pd.ExcelWriter(output_excel_file_path, engine='openpyxl') as writer:
        ...
        for row in range(2, len(results_df) + 2):  # 헤더 이후부터 시작
            ...
            # 전체 결과 컬럼에 대한 스타일 적용
            overall_result_cell = worksheet.cell(row=row, column=patient_id_col_idx + 1)
            if overall_result_cell.value == 'Pass':
                overall_result_cell.font = pass_font
            elif overall_result_cell.value == 'Fail':
                overall_result_cell.font = fail_font
            ...
            # 개별 ROI 결과 컬럼에 대한 스타일 적용
            for col in range(patient_id_col_idx + 2, len(results_df.columns) + 1):
                cell = worksheet.cell(row=row, column=col)
                cell.alignment = Alignment(horizontal='right')
                if cell.value == 'Pass':
                    cell.font = pass_font
        ...
        for roi in rois:
            ...
            for row in range(2, len(results_df) + 2):
                worksheet.cell(row=row, column=result_col).border = Border(left=bold_side, top=bold_side,
                                                                           bottom=bold_side)
                worksheet.cell(row=row, column=min_col).border = Border(top=bold_side, bottom=bold_side)
                worksheet.cell(row=row, column=system_col).border = Border(top=bold_side, bottom=bold_side)
                worksheet.cell(row=row, column=max_col).border = Border(right=bold_side, top=bold_side,
                                                                        bottom=bold_side)
